...
const USE_LABEL = Object.fromEntries(PATRIMONY_USE_TYPES.map((u) => [u.value, u.label]))

const TABS: { id: ReqStatus | 'all'; label: string }[] = [
...
function RequestCard({
  ...
  req:       PatrimonyWithdrawalRequest
  item:      PatrimonyItem | undefined
  session:   SessionUser
  ...
  return (
    ...
    >
      ...
      <div className="mb-3 grid grid-cols-2 gap-x-4 gap-y-1.5 text-[11px]">
        ...
        <div>
          <span style={{ color: '#3b5a7a' }}>Tipo de uso</span>
          <div style={{ color: '#8ba4bf' }}>{USE_LABEL[req.useType] ?? req.useType}</div>
        </div>
        <div>
        ...
        {req.expectedReturnDate && (
          <div>
            ...
            <div style={{ color: '#8ba4bf' }}>{req.expectedReturnDate}</div>
          </div>
        )}
        {req.projectOrClient && (
          <div>
            <span style={{ color: '#3b5a7a' }}>Projeto / Cliente</span>
          ...
          {rejecting ? (
            ...
            >
              ...
              <textarea
                ...
                rows={2}
                placeholder="Ex: Item em uso prioritário, data inválida..."
                value={rejectReason}
...
export function PatrimonyApprovalsPanel({ session }: { session: SessionUser }) {
  ...
  const approveMut = useApproveWithdrawalRequestMutation()
  const rejectMut  = useRejectWithdrawalRequestMutation()
  const cancelMut  = useCancelWithdrawalRequestMutation()
      ...
      <div className="mb-5 flex flex-wrap gap-1 rounded-xl p-1" style={{ background: '#0a0f1d', border: '1px solid rgba(255,255,255,0.07)' }}>
        {TABS.map((t) => {
          ...
          return (
            <button
              key={t.id}
              onClick={() => setActiveTab(t.id as ReqStatus)}
