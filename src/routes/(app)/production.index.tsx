...
function ProductionPage() {
  ...
  function filterCount(val: string) {
    if (val === 'pendente')   return requests.filter((r) => r.status === 'pending_approval').length
    if (val === 'disponivel') return items.filter((i) => i.availableQty > 0).length
    if (val === 'em_uso')     return items.filter((i) => i.usedQty > 0).length
    return items.filter((i) => i.status === val).length
  }
    ...
    <div className="animate-[fadeIn_0.3s_ease]">
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[13px]" style={{ color: '#3b5a7a' }}>
            Figurinos, adereços e itens de cena
            {totalItems > 0 && (
      ...
      {totalItems > 0 && (
        <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <StatCard label="Total de itens" value={totalItems}   color="#3b82f6" />
          <StatCard label="Disponíveis"    value={totalDisp}    color="#10b981" sub={`${totalAvailQty} un.`} />
          <StatCard label="Em uso"         value={totalInUse}   color="#f59e0b" sub={`${totalUsedQty} un.`} />
          <StatCard
            label="Pendentes"
            value={pendingCount}
            color={pendingCount > 0 ? '#f59e0b' : '#4a6380'}
            sub={pendingCount > 0 ? 'aguardando aprovação' : undefined}
        ...
        <>
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2">
            ...
            <input
              ...
              placeholder="Buscar por nome, categoria, cor, código..."
          ...
          {filtered.length === 0 ? (
            <div className="rounded-xl py-16 text-center" style={{ border: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              <div className="text-[14px] font-medium" style={{ color: '#2b4266' }}>
                {search || filter !== 'all' ? 'Nenhum item encontrado' : 'Nenhum item cadastrado no acervo'}
              </div>
              {!search && filter === 'all' && (
                <button
                  onClick={() => setShowNew(true)}
                ...
                <table className="w-full">
                  <thead>
                    <tr style={{ background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                      ...
                      <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#3b5a7a' }}>Qtd</th>
                      <th className="hidden px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider lg:table-cell" style={{ color: '#3b5a7a' }}>Condição</th>
...
function StatCard({
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <div className="text-[22px] font-bold" style={{ color, fontFamily: "'Space Grotesk', sans-serif" }}>
        {value}
      </div>
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    background: isOut && !isIn
                      ? 'rgba(245,158,11,0.1)'
                      : isIn
                        ? 'rgba(16,185,129,0.1)'
                        : 'rgba(139,164,191,0.07)',
                  }}
                >
                  {isIn ? '↓' : isOut ? '↑' : '·'}
                </div>
