...
function PatrimonyPage() {
  ...
  const semFoto     = items.filter((i) => !i.mainImageUrl).length
  ...
  const atrasados   = items.filter((i) => {
    const active = i.status === 'em_uso' || i.status === 'emprestado'
    return false // placeholder — expectedReturnDate not on item level, would need join
  }).length
  ...
  return (
    <div className="animate-[fadeIn_0.3s_ease]">
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[13px]" style={{ color: '#3b5a7a' }}>
            ...
            {total > 0 && (
              <> · <span style={{ color: '#8ba4bf' }}>{total} {total === 1 ? 'item' : 'itens'}</span></>
            )}
          </p>
        </div>
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <StatCard label="Manutenção"     value={manutencao} color="#f59e0b" />
        <StatCard label="Atenção"        value={atencao}    color={atencao > 0 ? '#ef4444' : '#4a6380'}
          sub={atencao > 0 ? `${extraviado} extraviado(s), ${semResp} s/ resp.` : 'Tudo em ordem'} />
        <StatCard
          label="Valor total"
        ...
        >
          ...
          <div className="text-[12px]" style={{ color: '#ef4444' }}>
            ...
            {[
              ...
              semFoto > 0    && `${semFoto} item(ns) sem foto`,
            ].filter(Boolean).join(' · ')}
          </div>
        </div>
      )}
                ...
                {filtered.map((item, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Localização */}
                      <td className="hidden px-4 py-3 lg:table-cell">
                        <span className="text-[12px]" style={{ color: '#4a6380' }}>
                          {item.location ?? '—'}
                        </span>
                      </td>
      ...
      {showImport && (
        <ImportExcelModal type="patrimony" onClose={() => setShowImport(false)} />
      )}

      {editItem && (
        <EditPatrimonyModal item={editItem} onClose={() => setEditItem(null)} />
      )}

      {checkoutItem && (
        <PatrimonyCheckOutModal item={checkoutItem} onClose={() => setCheckoutItem(null)} />
      )}
